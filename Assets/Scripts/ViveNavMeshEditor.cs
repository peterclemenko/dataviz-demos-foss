...
public class ViveNavMeshEditor : Editor
{
	...
	public override void OnInspectorGUI ()
	{
		...
		//GUILayout.Label ("Visible Navmesh", bold_wrap);

		GUILayout.Label ("Read FIRST: ", bold_wrap);
		GUIStyle wrap = EditorStyles.label;
		wrap.wordWrap = true;
		GUILayout.Label (
			"Please bake a Navigation Mesh (NavMesh) in Unity before using. Go to Window > Navigation > Bake. The blue area will be where you can teleport  When you " +
			"are done, click \"Update Navmesh Data\" below.  This will update the graphic of the playable area" +
			"that the player will see in-game. Here you can also set size and angle of your desired NavMesh" +
			" To manually remove teleportable areas, simply place an obstructive object with a mesh renderer and re \"Bake\" the Navmesh. When finished, the object should be deactivated.\n",
			wrap);
		...
		if (GUILayout.Button ("Create Visible Navmesh")) {
			Undo.RecordObject (mesh, "Create Visible Navmesh");
		...
		GUI.enabled = HasMesh;
		if (GUILayout.Button ("Erase Visual Navmesh Data")) {
			Undo.RecordObject (mesh, "Erase Visual Navmesh Data");
		...
		GUILayout.Label (HasMesh ? "NavMesh Ready" : "No NavMesh. Please check your settings");
